...
def test(steelType, stoveNum, input_factors, output_factors):
    df_input, df_output = 
...
class BpModelTest():
    ...
    def plot_result(self, output_list):
        ...
        data_original = pd.DataFrame(self.inverse_transform(data_original))
        ...
        for i, val in enumerate(self.output_factors):
            ...
            acc = self.accuracy(data_predicted.iloc[:, i], data_original.iloc[:, i])
            # acc = LinearRegression.score(np.asarray(data_original.iloc[:, i]),np.asarray(data_predicted.iloc[:, i]))
            # print(np.asarray(data_predicted.iloc[:, i]).shape)
            print('%s成分预测' %(val))
            print('准确度%.2f' %(acc))
