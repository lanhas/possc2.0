test_size = 0.15        # 训练集测试集划分比例，默认值0.2
val_size = 0.15         # 训练集验证集划分比例，默认值0.2
drop_nan_coef = 3       # 数据清洗系数，用于清洗缺失值数据，当单条记录中的空值超出设定的阈值时，删除该条数据，默认值3，值越大删去的数据越多
drop_err_coef = 3.5     # 数据清洗系数，用于清洗异常值数据，当单条记录中的数据超出某个阈值时， 删除该条记录，默认值3.5，值越大删去的数据越多
lr = 1e-4               # 训练步长
